...
export function SOONAIAssistant({ context, creatorData }: SOONAIProps) {
  ...
  const sendMessage = async (text: string) => {
    ...
    setMessages([...newMessages, { role: "assistant", content: data.reply ?? "SOON AI 暫時未能回覆，請稍後再試。" }]);
    setLoading(false);
  };
  ...
  return (
    <>
      <button
        ...
        title="SOON AI 助理"
        className="fixed bottom-6 right-6 z-50 flex h-14 w-14 items-center justify-center rounded-full bg-zinc-950 text-lg font-black text-[#f5a623] shadow-xl shadow-zinc-950/20 transition hover:-translate-y-0.5"
      >
        S
      </button>
      ...
      {isOpen && (
        <div className="fixed bottom-24 right-4 z-50 w-[calc(100vw-2rem)] overflow-hidden rounded-lg border border-zinc-200 bg-white shadow-2xl sm:right-6 sm:w-96">
          <div className="flex items-center justify-between bg-zinc-950 p-4 text-white">
            <div>
              <h3 className="font-bold">SOON AI 助理</h3>
              <p className="text-xs text-white/60">你的創作者事業智囊</p>
            </div>
          ...
          <div className="h-56 space-y-2 overflow-y-auto p-3">
            ...
            {loading && <div className="text-xs text-zinc-400">SOON AI 思考中...</div>}
          </div>
