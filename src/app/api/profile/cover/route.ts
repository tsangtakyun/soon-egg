...
async function ensureCoverBucket(supabase: SupabaseClient) {
  ...
  if (!bucket.public) {
    const { error } = await supabase.storage.updateBucket(COVER_BUCKET, {
      public: true,
      fileSizeLimit: MAX_FILE_SIZE,
      allowedMimeTypes: ALLOWED_MIME_TYPES,
    });
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    await ensureCoverBucket(serviceSupabase);
    ...
    const { data } = serviceSupabase.storage.from(COVER_BUCKET).getPublicUrl(path);
