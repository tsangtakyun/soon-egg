    ...
    <header className={`sticky top-0 z-50 bg-white/80 backdrop-blur-md transition ${scrolled ? "border-b border-black/10" : ""}`}>
      <nav className="mx-auto flex h-20 max-w-7xl items-center justify-between px-6">
        ...
        <div className="hidden items-center gap-6 sm:flex">
          ...
          <Link href="/signup" className="rounded-full bg-blue-500 px-5 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-blue-600">
            免費開始 →
          </Link>
        </div>
      ...
      {open && (
        <div className="border-t border-black/10 bg-white/90 px-6 py-4 backdrop-blur-md sm:hidden">
          <div className="mx-auto flex max-w-7xl flex-col gap-3">
            ...
            <Link href="/signup" className="rounded-full bg-blue-500 px-5 py-3 text-center text-sm font-semibold text-white transition-colors hover:bg-blue-600">免費開始 →</Link>
          </div>
        </div>
      )}
    </header>
