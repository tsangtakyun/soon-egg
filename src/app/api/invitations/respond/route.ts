...
export async function POST(req: Request) {
  ...
  if (!serverSupabase) {
    return NextResponse.json({ error: "系統設定未完成，請稍後再試。" }, { status: 500 });
  }

  const {
    data: { user },
  } = await serverSupabase.auth.getUser();
  if (!user) return NextResponse.json({ error: "請重新登入後再試。" }, { status: 401 });

  const body = (await req.json().catch(() => null)) as InvitationResponseBody | null;
  if (!body?.invitation_id || !body.status || !["accepted", "declined"].includes(body.status)) {
  ...
  if (!supabaseUrl || !serviceRoleKey || !cwBaseUrl || !internalKey) {
    console.error("Invitation response error: server integration env is missing");
    return NextResponse.json({ error: "合作同步服務暫時未完成設定。" }, { status: 503 });
  }
  ...
  const { data: profile, error: profileError } = await supabase
    ...
    .select("id,username,display_name,avatar_url,instagram_handle,instagram_followers")
    .eq("user_id", user.id)
    .single();
  if (profileError || !profile) {
    return NextResponse.json({ error: "找不到你的創作者檔案。" }, { status: 404 });
  }
  ...
  if (invitationError || !invitation) {
    return NextResponse.json({ error: "找不到呢個品牌邀請，請重新載入。" }, { status: 404 });
  }

  if (invitation.status === body.status) {
    return NextResponse.json({ success: true, status: body.status, unchanged: true });
  }
  if (invitation.status !== "pending") {
    return NextResponse.json({ error: "呢個邀請已經回覆，請重新載入。" }, { status: 409 });
  }
  ...
  if (!cwResponse) {
    return NextResponse.json({ error: "暫時未能連接品牌平台，本地狀態未有更改。請重試。" }, { status: 502 });
  }

  const cwData = await cwResponse.json().catch(() => null);
  if (!cwResponse.ok || cwData?.success === false) {
    console.error("Invitation response CW sync rejected:", cwResponse.status, cwData?.error);
    return NextResponse.json({ error: "品牌平台未能確認回覆，本地狀態未有更改。請重試。" }, { status: 502 });
  }
  ...
  if (updateError) {
    ...
    return NextResponse.json({ error: "品牌平台已收到回覆，但本地狀態更新失敗。請重新載入。" }, { status: 500 });
  }
  if (!updatedInvitation) {
    return NextResponse.json({ success: true, status: body.status, unchanged: true });
  }

  if (body.status === "accepted" && invitation.brand_name) {
    ...
    if (!existingPartner?.id) {
      const { error: partnerError } = await supabase.from("egg_brand_partners").insert({
        creator_id: profile.id,
        brand_name: invitation.brand_name,
      });
      if (partnerError) console.error("Invitation response partner sync failed:", partnerError);
    }
  ...
  return NextResponse.json({ success: true, status: body.status, responded_at: respondedAt });
}
