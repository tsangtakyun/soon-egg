  ...
  return (
    <article className="overflow-hidden rounded-xl border border-zinc-200 bg-white transition hover:shadow-md">
      {campaign.cover_image_url &&
        !campaign.cover_image_url.startsWith("/api/") && (
      ...
      <div className="p-4">
        <div className="mb-2 flex items-start justify-between gap-2">
          ...
          <span
            className={`flex-shrink-0 rounded-full px-2 py-0.5 text-xs ${campaign.status === "active" ? "bg-green-50 text-green-600" : "bg-yellow-50 text-yellow-600"}`}
          >
        ...
        <div className="mb-3 flex items-center gap-3 text-xs text-zinc-400">
          ...
          {campaign.duration_weeks && (
            <span>週期：{campaign.duration_weeks} 週</span>
          )}
        </div>
        {campaign.call_to_action && (
          <p className="mb-3 text-xs text-blue-600">
            目標：{campaign.call_to_action}
          </p>
        )}
        <div className="grid grid-cols-2 gap-2">
          <button
            onClick={onView}
            className="rounded-lg border border-zinc-200 py-2 text-sm font-medium text-zinc-600"
            type="button"
          >
            查看詳情
          </button>
          <button
            onClick={onApply}
            disabled={applied}
            className={`rounded-lg py-2 text-sm font-medium transition ${applied ? "cursor-not-allowed bg-zinc-100 text-zinc-400" : "bg-black text-white hover:bg-zinc-800"}`}
            type="button"
          >
            {applied ? "已申請" : "申請合作"}
          </button>
...
function CampaignDetailModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="max-h-[88vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-6">
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-2">
          <Detail label="合作預算" value={campaign.budget_range} />
          <Detail label="開始日期" value={campaign.starts_on} />
          <Detail
            label="合作週期"
            value={
              campaign.duration_weeks ? `${campaign.duration_weeks} 週` : null
            }
          />
          <Detail label="申請截止" value={campaign.application_deadline} />
        </div>
        <DetailBlock label="活動簡介" value={campaign.theme} />
        <DetailBlock label="品牌介紹" value={campaign.brand_overview} />
        <DetailBlock label="目標受眾" value={campaign.target_audience} />
        <DetailBlock
          label="合作內容"
          value={campaign.collab_formats?.join("、")}
        />
        <DetailBlock label="行動目標" value={campaign.call_to_action} />
        ...
        <button
          onClick={onApply}
          disabled={applied}
          className="mt-6 w-full rounded-xl bg-black py-3 text-sm font-semibold text-white disabled:bg-zinc-100 disabled:text-zinc-400"
        >
          {applied ? "已申請" : "申請合作"}
        </button>
      </div>
...
function Detail({ label, value }: { label: string; value?: string | null }) {
  return (
    <div className="rounded-xl bg-zinc-50 p-3">
      ...
      <p className="mt-1 text-sm text-zinc-800">{value || "未提供"}</p>
    </div>
  );
}
  ...
  async function handleApply() {
    ...
    const res = await fetch("/api/campaigns/apply", {
      ...
      body: JSON.stringify({
        ...
        collab_formats: campaign.collab_formats,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="w-full max-w-lg rounded-2xl bg-white p-6">
        ...
        <p className="mb-4 text-xs leading-relaxed text-zinc-400">
          申請後，品牌主會在 sooncreator.network 收到你的 Media Kit 連結。
        </p>
        {error && (
          <p className="mb-4 rounded-xl bg-red-50 px-3 py-2 text-xs text-red-600">
            {error}
          </p>
        )}
        <div className="flex gap-3">
          ...
          <button
            onClick={onClose}
            className="flex-1 rounded-xl border border-zinc-200 py-2.5 text-sm text-zinc-500"
            type="button"
          >
            取消
          </button>
...
function InvitationCard({
  ...
  return (
    <article
      className={`overflow-hidden rounded-xl border border-zinc-200 bg-white ${!isPending ? "opacity-60" : ""}`}
    >
      {invitation.cover_image_url &&
        !invitation.cover_image_url.startsWith("/api/") && (
          // eslint-disable-next-line @next/next/no-img-element
          <img
            src={invitation.cover_image_url}
            className="h-36 w-full object-cover"
            alt={invitation.campaign_name ?? ""}
          />
        )}
      <div className="p-4">
        <div className="mb-3 flex items-start justify-between gap-2">
          ...
          <span
            className={`flex-shrink-0 rounded-full px-2 py-0.5 text-xs ${invitation.status === "pending" ? "bg-yellow-50 text-yellow-600" : invitation.status === "accepted" ? "bg-green-50 text-green-600" : "bg-zinc-100 text-zinc-400"}`}
          >
        ...
        {invitation.brand_overview && (
          <div className="mb-3 rounded-lg bg-blue-50 px-3 py-2">
            ...
            <p className="line-clamp-3 text-xs text-blue-800">
              {invitation.brand_overview}
            </p>
          </div>
        )}
        ...
        <div className="mb-3 flex items-center gap-4 text-xs text-zinc-400">
          {invitation.starts_on && (
            <span>開始日期：{invitation.starts_on}</span>
          )}
          {invitation.duration_weeks && (
            <span>{invitation.duration_weeks} 週</span>
          )}
        </div>
        {invitation.budget_range && (
          <p className="mb-3 text-xs text-zinc-500">
            預算：{invitation.budget_range}
          </p>
        )}
        {invitation.call_to_action && (
          <p className="mb-3 text-xs text-blue-600">
            {invitation.call_to_action}
          </p>
        )}
        ...
        {error && (
          <div
            className="mb-3 rounded-lg bg-red-50 px-3 py-2 text-xs leading-5 text-red-600"
            role="alert"
          >
            {error}
          </div>
        ...
        {isPending ? (
          <div className="mt-2 flex gap-2">
            <button
              ...
              disabled={loading}
              className="flex-1 rounded-lg bg-black py-2 text-sm font-medium text-white disabled:cursor-wait disabled:opacity-50"
              type="button"
            >
              {loading ? "同步中..." : "接受邀請"}
            </button>
            <button
              ...
              disabled={loading}
              className="flex-1 rounded-lg border border-zinc-200 py-2 text-sm text-zinc-500 disabled:cursor-wait disabled:opacity-50"
              type="button"
            >
              婉拒
            </button>
  ...
  return (
    <article className="flex items-start gap-4 rounded-xl border border-zinc-200 bg-white p-4">
      ...
      <div className="flex-1">
        <h3 className="text-sm font-medium text-zinc-950">
          {invitation.brand_name || "未命名品牌"}
        </h3>
        <p className="mt-0.5 text-xs text-zinc-400">
          {invitation.campaign_name || "未命名 Campaign"}
        </p>
        {invitation.collab_formats && invitation.collab_formats.length > 0 && (
        ...
        {invitation.responded_at && (
          <p className="mt-2 text-xs text-zinc-300">
            已合作{" "}
            {new Date(invitation.responded_at).toLocaleDateString("zh-HK")}
          </p>
        )}
        <p className="mt-1 text-xs text-green-500">✓ 已加入 Media Kit</p>
      </div>
      <span className="flex-shrink-0 rounded-full bg-green-50 px-2 py-0.5 text-xs text-green-600">
        已合作
      </span>
    </article>
  );
...
function CampaignFeed({ profile }: { profile: Profile }) {
  ...
  if (campaigns.length === 0)
    return (
      <Empty text="暫時未有公開合作機會。完善創作者檔案後，新活動會顯示喺呢度。" />
    );
  return (
    <>
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        {campaigns.map((campaign) => (
          <CampaignCard
            ...
            onView={() => setDetailCampaign(campaign)}
      ...
      {detailCampaign && (
        <CampaignDetailModal
          ...
          applied={applications.includes(detailCampaign.id)}
          onClose={() => setDetailCampaign(null)}
          onApply={() => {
            setDetailCampaign(null);
            setShowPitchModal(detailCampaign);
          }}
        />
      )}
      {showPitchModal && (
      ...
      <div className="mb-6 flex overflow-x-auto border-b border-zinc-200">
        {tabs.map((tab) => (
          <button
            ...
            className={`flex shrink-0 items-center gap-2 whitespace-nowrap border-b-2 px-4 py-3 text-sm font-medium transition sm:px-6 ${activeTab === tab.id ? "border-black text-black" : "border-transparent text-zinc-400"}`}
            type="button"
          >
            {tab.label}
