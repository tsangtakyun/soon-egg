...
export async function GET() {
  const { user, workspaces, activeWorkspace } =
...
export async function PATCH(request: Request) {
  const { user, workspaces, admin } = await getCreatorWorkspaceContext();
  if (!user)
    return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  ...
  (await cookies()).set(ACTIVE_CREATOR_COOKIE, workspaceId, {
    httpOnly: true,
    sameSite: "lax",
    secure: true,
    path: "/",
  ...
  const { error } = await admin.auth.admin.updateUserById(user.id, {
    user_metadata: {
      ...user.user_metadata,
      egg_active_workspace_id: workspaceId,
    },
  });
  if (error) {
    console.error(
      "Creator workspace preference update failed",
      user.id,
      error.message,
    );
    return NextResponse.json(
...
export async function POST(request: Request) {
  ...
  if (!canCreateCreatorWorkspace(user.email))
    return NextResponse.json(
      { error: "只有內部管理員可以建立工作空間" },
      { status: 403 },
    );
  const body = await request.json().catch(() => ({}));
  const name = typeof body.name === "string" ? body.name.trim() : "";
  if (!name || name.length > 80)
    return NextResponse.json({ error: "請輸入有效名稱" }, { status: 400 });
  ...
  const { error: membershipError } = await admin
    ...
    .insert({
      ...
      user_id: user.id,
      email: user.email?.toLowerCase() ?? `${user.id}@workspace.local`,
      role: "owner",
    });
  if (membershipError) {
    console.error(
      "Creator workspace owner membership failed",
      profile.id,
      membershipError.message,
    );
    await admin.from("egg_creator_profiles").delete().eq("id", profile.id);
    return NextResponse.json(
      { error: "建立工作空間失敗，請稍後再試" },
      { status: 500 },
    );
  }
  (await cookies()).set(ACTIVE_CREATOR_COOKIE, profile.id, {
    httpOnly: true,
    sameSite: "lax",
    secure: true,
    path: "/",
  ...
  await admin.auth.admin.updateUserById(user.id, {
    user_metadata: {
      ...user.user_metadata,
      egg_active_workspace_id: profile.id,
    },
  });
...
export async function DELETE(request: Request) {
  ...
  if (!target)
    return NextResponse.json({ error: "找不到工作空間" }, { status: 404 });
  ...
  if (error) {
    console.error(
      "Creator workspace deletion failed",
      target.id,
      error.message,
    );
    return NextResponse.json(
  ...
  if (activeWorkspace?.id === target.id) {
    ...
    if (fallback) {
      (await cookies()).set(ACTIVE_CREATOR_COOKIE, fallback.id, {
        httpOnly: true,
        sameSite: "lax",
        secure: true,
        path: "/",
      ...
      await admin.auth.admin.updateUserById(user.id, {
        user_metadata: {
          ...user.user_metadata,
          egg_active_workspace_id: fallback.id,
        },
      });
    }
  }
