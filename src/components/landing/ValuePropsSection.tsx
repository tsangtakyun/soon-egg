...
const props = [
  { icon: "🛍️", title: "推動銷售", body: "直接在 Link in Bio 賣數位產品、課程、預約、聯盟連結。最快一日內收款。" },
  { icon: "💰", title: "獲得報酬", body: "把聯盟佣金升級為正式品牌合作。直接 pitch 亞洲品牌，用 Media Kit 談到理想價錢。" },
  { icon: "🤝", title: "品牌合作", body: "MOON 幫你搵品牌、評估合適度、生成廣東話 pitch，令你的收件箱變成合作機會。" },
];
        ...
        <div className="mt-12 grid gap-6 lg:grid-cols-3">
          {props.map((item) => (
            <article key={item.title} className="rounded-2xl border border-black/10 bg-[#fafafa] p-7">
              <div className="flex h-14 w-14 items-center justify-center rounded-full bg-[#f5a623]/18 text-3xl">{item.icon}</div>
